...
def _create_results_table(db: sqlite3.Connection):
    db.execute("CREATE TABLE results (time, molfile_id UNIQUE, result)")


def remove_consumer_column(db: sqlite3.Connection):
    db.execute("PRAGMA foreign_keys=off")
    db.execute("BEGIN TRANSACTION")
    db.execute("CREATE TABLE new_results AS SELECT time, molfile_id, result FROM results")
    db.execute("DROP TABLE results")
    db.execute("ALTER TABLE new_results RENAME TO results")
    db.execute("COMMIT")
...
def regression(
    ...
    with (
        ...
        remove_consumer_column(intermediate_log_db)
